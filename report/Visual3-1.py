...
writer = imageio.get_writer('gravitational_field_simulation.mp4', fps=20)

# Screen capture setup using mss
with mss.mss() as sct:
    # Define the bounding box for the capture
    monitor = {"top": scene.y, "left": scene.x, "width": scene.width, "height": scene.height}
    
    frame_number = 0
    while frame_number < 200:  # Limit the number of frames to capture
        rate(50)
        ...
        # Capture the screen and append it to the video
